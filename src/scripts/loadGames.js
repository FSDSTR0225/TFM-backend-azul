...
const fetchAndSaveGames = async () => {
  ...
  let nextPageUrl = `https://api.rawg.io/api/games?key=${process.env.RAWG_API_KEY}&page=1&page_size=10`;

 let pagesFetched = 0; // Contador de paginas fetched;
 const maxPages = 10; // Limite de paginas a fetchear;

  while (nextPageUrl && pagesFetched < maxPages) {
    const games = await fetchGames(nextPageUrl);

    if(!games){
      break;
    }
    for (const gameData of games) {
      await saveGames(gameData);
    }

    
    nextPageUrl= games.next;
    pagesFetched++;
    console.log(`${pagesFetched} Paginas guardadas con éxito`);
    await Game.insertMany(games);
  }
await mongoose.disconnect();
console.log("Juegos guardados con éxito");
};
